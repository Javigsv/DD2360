...
/**
 * Checks gpu_grayscale<<<grid, block>>>(bitmap.width, bitmap.height,
                                       d_bitmap, d_image_out[0]);
        
        cudaMemcpy(image_out[0], d_image_out[0],
         if there has been any CUDA error. The method will automatically print
